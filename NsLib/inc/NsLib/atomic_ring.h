#ifndef NS_ATOMIC_RING_H
#define NS_ATOMIC_RING_H
...
namespace ns
{
    // Ring with limited thread safety...
    template<typename T>
    class atomic_ring
    {
    private:
    ...
    public:
        atomic_ring(const int cap) : capacity(cap), iBeg(0), iEnd(0), count(0)
        {
            data = std::unique_ptr<T[]>(new T[cap]);
        }

        atomic_ring(atomic_ring & other) = delete;
